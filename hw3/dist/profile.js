...
var defaultLabels = ["Jeffrey Wang", "Jhw5@rice.edu", "123-123-1234", "77005", "01/01/1998", "password"];
...
var defaultAlerts = ["","Please enter a valid email address", "Please enter your phone number in the following format: 123-123-1234",
    "Please enter a valid 5 digit zip code", "You follow the format mm/dd/yyyy and you must be 18 years old to register!", "Your password does not match!"];
...
function checkRegex (idName, titleIndex) {
    ...
    if (boolean == false) {
        (idName === "password") ? alert(defaultAlerts[titleIndex+1]) :alert(defaultAlerts[titleIndex]);
        return false;
    } else {
        return true;
    }
...
function updateLabels(idName, titleIndex){
    ...
    if (inputVal.value != "") {
        ...
        } else if (checkRegex(idName, titleIndex)) {
